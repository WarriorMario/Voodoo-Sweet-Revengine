...
Graphics::Graphics(HWNDKey& key, HWNDKey& key2)
{
  ...
  god_window_buffer = reinterpret_cast<Color*>(
    _aligned_malloc(sizeof(Color) * Graphics::ScreenWidth * Graphics::ScreenHeight, 128u));
  // allocate memory for sysbuffer (16-byte aligned for faster access)
...
void Graphics::EndFrame()
{
...

  {
    ...
    PROFILE_SCOPE("Game::Go::EndFrame::GodWindow");
    ...
    {
      PROFILE_SCOPE("Game::Go::EndFrame::GodWindow::LineCopy");
    ...
    {
      PROFILE_SCOPE("Game::Go::EndFrame::GodWindow::Render");
    ...
    {
      PROFILE_SCOPE("Game::Go::EndFrame::GodWindow::FlipBuffers");
...

  {
    ...
    PROFILE_SCOPE("Game::Go::EndFrame::PlayerWindow");
    // lock and map the adapter memory for copying over the sysbuffer
    ...
    // setup parameters for copy operation
    Color* pDst = reinterpret_cast<Color*>(mappedSysBufferTexture2.pData);
    size_t dstPitch = mappedSysBufferTexture2.RowPitch / sizeof(Color);
    size_t srcPitch = Graphics::ScreenWidth;
    size_t rowBytes = srcPitch * sizeof(Color);
    {
      PROFILE_SCOPE("Game::Go::EndFrame::PlayerWindow::LineCopy");
      // perform the copy line-by-line
      for(size_t y = 0u; y < Graphics::ScreenHeight; y++)
      {
    ...
    {
      PROFILE_SCOPE("Game::Go::EndFrame::PlayerWindow::Render");
      // render offscreen scene texture to back buffer
