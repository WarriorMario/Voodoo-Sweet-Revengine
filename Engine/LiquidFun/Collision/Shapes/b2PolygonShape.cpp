...
void b2PolygonShape::SetAsBox(float32 hx, float32 hy, const Vec2& center, float32 angle)
{
	m_count = 4;
	m_vertices[0].Set(-hx, -hy);
	m_vertices[1].Set( hx, -hy);
...
void b2PolygonShape::Set(const Vec2* vertices, int32 count)
{
...
void b2PolygonShape::ComputeDistance(const b2Transform& xf, const Vec2& p, float32* distance, Vec2* normal, int32 childIndex) const
{
	...
	if (maxDistance > 0)
	{
		...
		for (int32 i = 0; i < m_count; ++i)
		{
			Vec2 distance = pLocal - m_vertices[i];
